...
# with no D-Bus session bus -- waybar and swaync silently down, swaybg and
# wlsunset fine. --check could not see any of it, because a file generated by a
# heredoc is in no map.
#
# (nm-applet and blueman-applet are deliberately not started at all, and
